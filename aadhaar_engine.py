...
class AadhaarEngine:
    ...
    async def close(self):
        ...
        try:
            ...
            # Also kill Phase 2 download process if active
            if hasattr(self, 'phase2_process') and self.phase2_process:
                try:
                    self.phase2_process.terminate()
                    print(f"🛑 [ENGINE] Terminated Phase 2 download process for {self.chat_id}.")
                except: pass
                self.phase2_process = None
        except Exception as e:
    ...
    async def run_uidai_phase(self, chat_id, eid, name, mobile, user_info=None):
        ...
        while current_retry < max_retries:
            ...
            try:
                ...
                process = await asyncio.create_subprocess_exec(
                    sys.executable, '-u', download_script, str(eid), str(chat_id), str(mobile),
                    stdin=asyncio.subprocess.PIPE,
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE
                )
                self.phase2_process = process  # Store for cancel/close()
                ...
                err_msg = str(e)
                # If user cancelled — stop immediately, do NOT retry
                if "cancelled by user" in err_msg.lower() or "__cancel__" in err_msg.lower():
                    raise e

                is_technical = "technical difficulties" in err_msg.lower()
