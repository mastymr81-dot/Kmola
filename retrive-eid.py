...
def run_retrieval(name, dob, mobile):
    ...
    for full_name in candidate_names:
        ...
        while attempt <= max_captcha_retries and captcha_attempts < max_captcha_attempts:
            ...
            try:
                ...
                print(f"[P1 FULL RESPONSE] {otp_res_json}")
                ...
                if msg and ("exceeded" in msg.lower() or "permissible limit" in msg.lower() or "retry after" in msg.lower()):
                    technical_diff = True
                    last_server_msg = f"⛔ OTP Limit Exceeded! UIDAI ne is number ke liye OTP block kar diya hai. Kripya 20-30 minutes baad try karein. ||| Real Server Response: {msg}"
                    break
    ...
    res_final = session.post(RETRIEVE_URL, json=final_payload, headers=headers, timeout=60)
